...
@platform aware function init_imfilter_kernel({accelerator_count::(@atleast 1), accelerator_api::OpenCL_API})
  ...
  function clkarray_create(img)
      ...
      # populate GPU memory
      (cl.Buffer(T, GPU.ctx, :copy, hostbuf=A), clpad, size(A), length(A))
  end
  ...
  struct CLKArray{T,N} <: AbstractArray{T,N}
      ...
      function CLKArray(img) 
          N = ndims(img)
          T = eltype(img)
          (bufA1, clpad, A_size, A_length) = clkarray_create(img)
          bufA2 = clkarray_create(img.^2)[1]
          new{T,N}(bufA1, bufA2, img, clpad, A_size, A_length)
      end
  
      function CLKArray(T, N, bufA, bufA_,img,clpad, A_size, A_length)
          new{T,N}(bufA, bufA_, img, clpad, A_size, A_length)
      end
  ...
  function power2(a::CLKArray{T,N}) where {T,N} 
      CLKArray(T, N, a.bufA_, a.bufA, a.img, a.clpad, a.A_size, a.A_length) 
  end
  ...
  #   A = parent(A)
     ...
     p_ = clfft.Plan(T, ctx, img.A_size)
